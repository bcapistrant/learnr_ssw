...
description: "This tutorial introduces R and how to work with data in R."
---
...
Welcome to your first module for our course!  Throughout this course, we will be working with data and working on how to communicate what we did with the data, what we found, and why that work matters.  One dimension of a high quality data analysis is that your steps and process are clear enough that someone else could reproduce it.  This replicability and clarity are arguably even more important when working with publicly available secondary data -- anyone has access to the data and should be able to repeat your steps. One way to do that is to keep the analysis and writing very closely connected. We will be using `R` as a way to keep the data analysis and writing of the paper within the same document.

This tutorial is a brief introduction about what `R` is, how it works, and start to use `R` to work with real data. The different sections of this tutorial will include text to read that explains concepts and new terminology, "quiz" questions to check you understanding of the material, and interactive segments where you will be practicing using `R` in real time. Since we will be working with real data, there will also be some discussion toward the end with an introduction to *datasets* we'll see in the course in addition to the introduction to `R`.

Specifically, after completing this tutorial, you will be able to:

- articulate what `R` is
- explain why `R` is useful for statistical analysis & writing up results
- define and use key terminology for `R`, like packages and functions
- employ functions to load a package and real data
- discuss principles of how data are organized and structured
- explain what makes a dataset "tidy".

In the next section, you will learn a little more about `R` before getting into working with it in the subsequent sections. 
...
If you subscribed to a *meal prep service* like Blue Apron, you might get raw ingredients (data) and also a set of instructions (functions). 
...
It may *seem* like nothing happened -- hopefully, you clicked *"Run Code"*, saw that box turn grey, a spinner spun for a while, and then the box turned colors back to blue -- but actually that "nothing happened" sense is by design: *the package got loaded on the back end without you even having to see it.*

To see if that worked, let's check to see if we can *so* something with the package -- like loading a dataset that's in the package.
...
This is a point where its helpful to remember that `R` is a programming language. Like many languages, there is a discernible syntax. In fact, I find it helpful to think about the English (or other language) analogues to the `R` language. 

Functions are the verb -- the function will *do* something, and that something is included in the parentheses that follow the name of the function. What you put inside the parentheses is sort of like the subject or object of the sentence. If `saute()` is the function, what are you sauteing? `saute(broccoli)` is like a command to saute the broccoli. 

If we want to take the language metaphor further, a function's verb tense is almost always the imperative -- it's a command. So like "Pass the salt," or "Wake her" where pass / wake are the verbs and the salt or her are the direct or indirect objects. The subject - you / `R` - is implied.
...
To see that the function above worked, use another function `head()` to show you the "head" of the data, or just the first 5 rows of data (there is a corollary function for the last 5 rows called `tail()`). 
...
Again, most of our examples will be where the "observations" are individual people, but they don't need to be. For example, in the table below, each row/observation is a different food, and each column/variable is some piece of nutritional information (serving size, calories, and grams of each macro-nutrient - fat, protein, carbohydrate).
...
- explained what `R` is
- introduced why `R` is useful for statistical analysis & writing up results
- discussed what `R` packages are
- explored what `R` functions are and what they do
- practiced using functions to load a package and real data
- introduced principles of how data are organized and structured
- defined principles of "tidy" data and its terminology (observations/rows, variables/columns, values)

## Supplemental Reading

Want to learn more about `R` and why others use it? Below are some excellent supplemental readings. Some of these readings are more (B)eginner friendly and others are more (A)dvanced, and so we have marked each reading appropriately. 
...
3. $^A$ McNamara, A. (2020, July). [Speaking R.](https://www.amelia.mn/SpeakingR/#1). [Conference presentation]. [useR! 2020, Remote Conference](https://user2020.r-project.org/). https://www.amelia.mn/SpeakingR/#1
